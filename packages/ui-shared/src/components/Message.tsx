 ...
 * 
 * Supports both user and assistant messages, with optional feedback
 * buttons and handoff email submission for assistant messages.
 */
export function Message({
  ...
  if (isHandoff && m.handoffRequestId && onSubmitEmail) {
    return (
      <div className={`cbw-msg-row assistant ${classNames.row || ''}`}>
        ...
        <div className={`cbw-msg assistant cbw-handoff-card ${classNames.bubble || ''}`}>
          <div className="cbw-handoff-glass-circle"></div>

          {submitted ? (
            <div className="cbw-handoff-success">
              <div className="cbw-handoff-success-sparkle">✨</div>
              <div className="cbw-handoff-success-title">Talebiniz alındı!</div>
            ...
            <>
              <div className="cbw-handoff-header">
                ...
                <span className="cbw-handoff-title">Destek Talebi</span>
              </div>
          ...
          <div className={`cbw-handoff-ts ${classNames.timestamp || ''}`}>{time}</div>
        </div>
      </div>
    )
  }
  ...
  // Regular message
  return (
    <div className={`cbw-msg-row ${isUser ? 'user' : 'assistant'} ${classNames.row || ''}`}>
      {!isUser && (
        <div className={`cbw-avatar ${classNames.avatar || ''}`}>
          {botIcon ? (
            <img src={botIcon} alt="" className="cbw-avatar-img" />
      ...
      <div className={`cbw-msg ${m.role} ${classNames.bubble || ''}`}>
        <div className={`cbw-msg-content ${classNames.content || ''}`}>
          <Markdown options={{ createElement }}>{m.content}</Markdown>
        </div>
        <div className={`cbw-msg-footer ${classNames.footer || ''}`}>
          {!isUser && m.id && onFeedback && (
            <div className={`cbw-feedback-container ${classNames.feedbackContainer || ''}`}>
              <button
              ...
              >
                <svg
                  ...
                  height="14"
                  viewBox="0 0 24 24"
                  fill={m.feedback === true ? 'currentColor' : 'none'}
                  stroke="currentColor"
                  strokeWidth="2.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
              ...
              >
                <svg
                  ...
                  height="14"
                  viewBox="0 0 24 24"
                  fill={m.feedback === false ? 'currentColor' : 'none'}
                  stroke="currentColor"
                  strokeWidth="2.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
          ...
          <div className={`cbw-ts ${classNames.timestamp || ''}`}>{time}</div>
        </div>
      </div>
