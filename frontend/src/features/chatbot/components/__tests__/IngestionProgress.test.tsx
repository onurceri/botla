...
describe('IngestionProgress', () => {
  ...
  it('shows correct status text for processing', () => {
    const { container } = render(
      <IngestionProgress
        sources={[{ id: '1', source_type: 'pdf', name: 'test.pdf', status: 'processing' }]}
      />,
    )
  ...
  it('shows correct status text for queued', () => {
    const { container } = render(
      <IngestionProgress
        sources={[{ id: '1', source_type: 'pdf', name: 'test.pdf', status: 'queued' }]}
      />,
    )
  ...
  it('shows correct status text for completed', () => {
    const { container } = render(
      <IngestionProgress
        sources={[{ id: '1', source_type: 'pdf', name: 'test.pdf', status: 'completed' }]}
      />,
    )
  ...
  it('shows correct status text for failed', () => {
    const { container } = render(
      <IngestionProgress
        sources={[{ id: '1', source_type: 'pdf', name: 'test.pdf', status: 'failed' }]}
      />,
    )
  ...
  it('applies custom className', () => {
    const { container } = render(
      <IngestionProgress sources={mockSources()} className="test-class" />,
    )
