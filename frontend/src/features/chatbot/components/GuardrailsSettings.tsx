    ...
    <div className="space-y-6">
      <Tabs defaultValue="thresholds" className="w-full">
        <TabsList className="w-full grid grid-cols-1 sm:grid-cols-3 h-auto sm:h-12 p-1 bg-muted/50 rounded-xl gap-1 sm:gap-0">
          <TabsTrigger
            value="thresholds"
            className="rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm transition-all py-2 sm:py-1"
          >
              ...
              <div className="space-y-3 bg-muted/30 p-6 rounded-2xl border border-border/50">
                ...
                <div className="relative h-12 w-full rounded-xl overflow-hidden flex shadow-sm ring-1 ring-black/5">
                  ...
                  >
                    ...
                    <span className="text-[10px] text-white/80 font-medium hidden group-hover:block absolute bottom-1 whitespace-nowrap">
                      Cevap Vermez
                    </span>
                  </div>
                  {/* Yellow Zone */}
                  ...
                  >
                    ...
                    <span className="text-[10px] text-white/90 font-medium hidden group-hover:block absolute bottom-1 whitespace-nowrap">
                      Kontrol Eder
                    </span>
                  </div>
                  {/* Green Zone */}
                  ...
                  <div className="flex justify-between items-start">
                    <div className="space-y-1.5">
                      ...
                      <p className="text-xs text-muted-foreground leading-relaxed">
                        Bu skorun altındaki eşleşmeler tamamen reddedilir ve "Bilgi Yok" mesajı
                        gösterilir.
                      </p>
                    </div>
                  </div>
                ...
                <div className="space-y-5 p-5 border rounded-2xl bg-card hover:border-emerald-200 transition-colors shadow-sm">
                  <div className="flex justify-between items-start">
                    <div className="space-y-1.5">
                      ...
                      <p className="text-xs text-muted-foreground leading-relaxed">
                        Bu skorun üzerindeki eşleşmeler güvenilir kabul edilir ve doğrudan
                        kullanıcıya sunulur.
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between p-4 border rounded-xl bg-muted/20">
                  <div className="space-y-1 pr-4">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Sarı bölgeye düşen (şüpheli) sonuçlarda kullanıcıya cevabın kesin
                      olmayabileceğini belirt.
                    </p>
                  </div>
                  <Switch
                ...
                <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <span className="text-muted-foreground text-sm">
                      Güven eşiklerini özelleştirmek ve gelişmiş ayarları açmak için planınızı
                      yükseltin.
                    </span>
                  </div>
                  <Button
                    variant="outline"
                    size="sm"
                    className="ml-auto border-primary/20 hover:bg-primary/10 text-primary"
                  >
                    Yükselt
                  </Button>
                </div>
            ...
            <CardHeader className="bg-muted/30 pb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription className="text-base mt-1">
                    Beklenmedik durumlarda veya hata anında botun vereceği standart cevapları
                    belirleyin.
                  </CardDescription>
                </div>
              </div>
              ...
              <div className="grid gap-8">
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    {!canCustomizeMessages && (
                      <Badge
                        variant="outline"
                        className="text-[10px] text-muted-foreground border-primary/20 bg-primary/5"
                      >
                        Plan Yükselt
                      </Badge>
                    )}
                  ...
                  <p className="text-sm text-muted-foreground">
                    Bu mesaj, bot kullanıcının sorusuna veritabanında yeterli bir cevap
                    bulamadığında ("Sabit Mesaj" modu seçiliyse) gösterilir.
                  </p>
                </div>
              ...
              {!topicRestrictions ? (
                <div className="flex flex-col items-center justify-center py-12 text-center border-2 border-dashed rounded-2xl bg-muted/10 space-y-4">
                  ...
                  <div className="max-w-md space-y-2">
                    ...
                    <p className="text-muted-foreground">
                      Şu anda konu sınırlaması aktif değil. Bot, bilgi bankasındaki içeriklere
                      dayanarak her türlü soruya cevap vermeye çalışır.
                    </p>
                  </div>
                  <Button
                    onClick={() => toggleTopicRestrictions(true)}
                    disabled={!canManageTopics}
                    className="mt-4"
                  >
                ...
                <div className="space-y-8 animate-in slide-in-from-top-4 duration-300">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-muted-foreground">
                        Bu konularda asla cevap verilmez.
                      </span>
                    </div>
                    ...
                    <div className="bg-muted/30 rounded-xl p-4 min-h-[80px] border border-border/50">
                      {!topicRestrictions.blocked_topics ||
                      topicRestrictions.blocked_topics.length === 0 ? (
                        ...
                        <div className="flex flex-wrap gap-2">
                          {topicRestrictions.blocked_topics?.map((topic, i) => (
                            <Badge
                              key={i}
                              variant="destructive"
                              className="pl-3 pr-1.5 py-1.5 text-sm hover:bg-red-600 transition-colors shadow-sm"
                            >
                              {topic}
                              <button
                                type="button"
                                onClick={() => removeTopic('blocked', i)}
                                className="hover:bg-red-700/50 rounded-full p-0.5 ml-2 transition-colors"
                              >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-muted/30 rounded-xl p-4 min-h-[80px] border border-border/50">
                      {!topicRestrictions.allowed_topics ||
                      topicRestrictions.allowed_topics.length === 0 ? (
                        <div className="flex items-center justify-center h-full text-sm text-muted-foreground italic">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {topicRestrictions.allowed_topics?.map((topic, i) => (
                            <Badge
                              key={i}
                              variant="secondary"
                              className="pl-3 pr-1.5 py-1.5 text-sm bg-background border shadow-sm hover:bg-accent transition-colors text-foreground"
                            >
                              {topic}
                              <button
                                type="button"
                                onClick={() => removeTopic('allowed', i)}
                                className="hover:bg-muted-foreground/20 rounded-full p-0.5 ml-2 transition-colors"
                              >
                                <X className="w-3.5 h-3.5" />
                              </button>
                            </Badge>
                    ...
                    <p className="text-xs text-muted-foreground mt-2">
                      * Eğer "İzin Verilen Konular" listesine ekleme yaparsanız, bot{' '}
                      <strong>SADECE</strong> bu konular hakkında konuşur, diğer her şeyi reddeder.
                    </p>
                  </div>
                </div>
              )}
