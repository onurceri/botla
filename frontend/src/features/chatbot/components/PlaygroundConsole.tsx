...
import { Terminal, ChevronUp, ChevronDown, Trash2, Info, AlertCircle, MessageSquare, Bot, Cpu } from 'lucide-react'
...
export default function PlaygroundConsole() {
  ...
  const getMessageForType = (type: string, payload: any): string => {
    switch (type) {
      case 'config_loaded': return 'Widget yapılandırması başarıyla yüklendi.'
      case 'message_sent': return `Kullanıcı mesajı: "${payload.content}"`
      case 'response_received': return `Bot yanıtı: "${payload.content}"`
      case 'error': return `Hata oluştu: ${payload.message}`
      case 'handoff': return `Canlı desteğe yönlendirme tetiklendi.`
      default: return `${type} olayı tetiklendi.`
    }
  }

  const getIconForType = (type: string) => {
    switch (type) {
      case 'config_loaded': return <Cpu className="w-3.5 h-3.5 text-blue-400" />
      case 'message_sent': return <MessageSquare className="w-3.5 h-3.5 text-slate-400" />
      case 'response_received': return <Bot className="w-3.5 h-3.5 text-emerald-400" />
      case 'error': return <AlertCircle className="w-3.5 h-3.5 text-red-400" />
      case 'handoff': return <Info className="w-3.5 h-3.5 text-amber-400" />
        ...
        >
          {logs.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-600 gap-3">
              ...
              <p className="italic text-[12px]">Henüz olay kaydedilmedi. Widget ile etkileşime geçin.</p>
            </div>
          ) : (
            <div className="space-y-2.5">
              {logs.map(log => (
                <div key={log.id} className="flex gap-4 group/log animate-in fade-in slide-in-from-left-2 duration-300">
                  <span className="text-slate-600 shrink-0 select-none font-medium opacity-70">
                    [{log.timestamp.toLocaleTimeString([], { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' })}]
                  </span>
                  <div className="flex items-start gap-2.5 min-w-0">
                    <span className="mt-0.5 shrink-0 opacity-80 group-hover/log:opacity-100 transition-opacity">{getIconForType(log.type)}</span>
                    <div className="flex flex-col gap-1 min-w-0">
                      <span className={`break-words font-medium ${
                        log.type === 'error' ? 'text-red-400' : 
                        log.type === 'response_received' ? 'text-emerald-400' : 
                        log.type === 'message_sent' ? 'text-slate-200' : 
                        'text-slate-300'
                      }`}>
                        {log.message}
                      </span>
