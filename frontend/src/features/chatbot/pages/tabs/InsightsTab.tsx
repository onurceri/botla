        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Performans analizleri ve destek talepleri.
            </p>
          </div>
        </div>
      </div>

      <Tabs value={activeSection} onValueChange={(v) => setActiveSection(v as 'analytics' | 'requests')} className="w-full">
        <TabsList className="w-full justify-start bg-muted/50 p-1 h-auto">
          <TabsTrigger value="analytics" className="flex items-center gap-2 px-4 py-2.5 data-[state=active]:bg-background">
            <BarChart3 className="w-4 h-4" />
            Analizler
          </TabsTrigger>
          {canUseHandoff && (
            <TabsTrigger value="requests" className="flex items-center gap-2 px-4 py-2.5 data-[state=active]:bg-background">
