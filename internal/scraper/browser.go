...
func allowed(urlStr string, allowed []string) bool {
    ...
    host := u.Host
    if h, p, err := net.SplitHostPort(u.Host); err == nil && h != "" { host = h; _ = p }
    h := strings.ToLower(host)
    for _, d := range allowed {
        d = strings.ToLower(strings.TrimSpace(d))
        if h == d || strings.HasSuffix(h, "."+d) { return true }
    }
    return false
}
...
func ScrapeDynamicURL(urlStr string, cfg DynamicConfig) (string, error) {
    ...
    html, err := p.HTML()
    if err != nil { return "", err }
    if len(html) > 2<<20 { html = html[:2<<20] }
