...
func TestNewOpenAIClientFromEnv(t *testing.T) {
    ...
    if err != nil || c == nil { t.Fatalf("client err: %v", err) }
}
...
func TestCreateEmbedding_Success(t *testing.T) {
    ...
    if err != nil || len(v) != 2 { t.Fatalf("embedding err: %v", err) }
}
...
func TestCreateEmbeddingsBatch_Success(t *testing.T) {
    ...
    if err != nil || len(v) != 2 { t.Fatalf("batch err: %v", err) }
}
...
func TestCreateCompletion_Success(t *testing.T) {
    ...
    if err != nil || ans == "" || tokens == 0 { t.Fatalf("completion err: %v", err) }
}
