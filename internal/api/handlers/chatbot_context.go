...
func getSourceContext(w http.ResponseWriter, r *http.Request, dbConn *sql.DB, wsService *services.WorkspaceService, orgService *services.OrganizationService) (*models.DataSource, *models.Chatbot, string, bool) {
	userID, ok := middleware.UserIDFromContext(r.Context())
	if !ok || userID == "" {
		w.WriteHeader(http.StatusUnauthorized)
