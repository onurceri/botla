...
func (h *MeHandlers) Me(w http.ResponseWriter, r *http.Request) {
    ...
    if !ok || uid == "" { w.WriteHeader(http.StatusUnauthorized); return }
    u, err := db.GetUserByID(r.Context(), h.DB, uid)
    if err != nil { w.WriteHeader(http.StatusInternalServerError); return }
    if u == nil { w.WriteHeader(http.StatusNotFound); return }
    var fullName *string
    if u.FullName.Valid { fullName = &u.FullName.String }
    var avatar *string
    if u.AvatarURL.Valid { avatar = &u.AvatarURL.String }
    plan := "free"
    if u.SubscriptionPlan.Valid && u.SubscriptionPlan.String != "" { plan = u.SubscriptionPlan.String }
