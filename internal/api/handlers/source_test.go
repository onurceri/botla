...
func TestIsPDFContentType(t *testing.T) {
	if !isPDFContentType("application/pdf", "x.txt") {
		t.Fatal("ct should pass")
	}
	if !isPDFContentType("", "x.pdf") {
		t.Fatal("suffix should pass")
	}
	if isPDFContentType("text/plain", "x.txt") {
		t.Fatal("should fail")
	}
}
