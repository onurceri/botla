...
func TestSources_ETag_Status(t *testing.T) {
    ...
    ctx := func(req *http.Request) *http.Request { return req.WithContext(context.WithValue(req.Context(), middleware.ContextKeyUserID, uid)) }
    ...
    if rr2.Code != http.StatusCreated { t.Fatalf("create src: %d", rr2.Code) }
    ...
    if rr3.Code != http.StatusOK { t.Fatalf("status1: %d", rr3.Code) }
    etag := rr3.Header().Get("ETag")
    if etag == "" { t.Fatalf("missing etag") }
    ...
    if rr4.Code != http.StatusNotModified { t.Fatalf("expected 304, got %d", rr4.Code) }
}
