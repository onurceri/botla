...
func TestDefaultHelpers(t *testing.T) {
    if defaultString(nil, "d") != "d" { t.Fatal("defaultString failed") }
    s := " x "
    if defaultString(&s, "d") != "x" { t.Fatal("defaultString trim failed") }
    if defaultInt(nil, 3) != 3 { t.Fatal("defaultInt failed") }
    i := 7
    if defaultInt(&i, 3) != 7 { t.Fatal("defaultInt override failed") }
    if defaultFloat32(nil, 0.5) != 0.5 { t.Fatal("defaultFloat32 failed") }
    f := float32(0.9)
    if defaultFloat32(&f, 0.5) != 0.9 { t.Fatal("defaultFloat32 override failed") }
}
