...
func TestAuth_RefreshRotationAndLogout(t *testing.T) {
    ...
    if resRef1.StatusCode != http.StatusOK { t.Fatalf("expected 200, got %d", resRef1.StatusCode) }
    var tp2 tokenPair
    json.NewDecoder(resRef1.Body).Decode(&tp2)
    resRef1.Body.Close()
    if tp2.RefreshToken == "" { t.Fatalf("missing new refresh token") }
    ...
    if resRefOld.StatusCode != http.StatusUnauthorized { t.Fatalf("expected 401 for revoked, got %d", resRefOld.StatusCode) }
    ...
    if resLogout.StatusCode != http.StatusOK { t.Fatalf("expected 200 on logout, got %d", resLogout.StatusCode) }
    resLogout.Body.Close()

    resRef3, _ := http.Post(te.Server.URL+"/api/v1/auth/refresh", "application/json", bytes.NewReader(lrj))
    if resRef3.StatusCode != http.StatusUnauthorized { t.Fatalf("expected 401 after logout, got %d", resRef3.StatusCode) }
    resRef3.Body.Close()
}
