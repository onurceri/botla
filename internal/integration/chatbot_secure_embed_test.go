...
func TestChatbot_SecureEmbed_UpdateAndGet(t *testing.T) {
    ...
    if resL.StatusCode != http.StatusOK { t.Fatalf("list expected 200, got %d", resL.StatusCode) }
    var items []map[string]any
    json.NewDecoder(resL.Body).Decode(&items)
    resL.Body.Close()
    if len(items) == 0 { t.Fatalf("no chatbots listed") }
    ...
    if resU.StatusCode != http.StatusOK { t.Fatalf("expected 200, got %d", resU.StatusCode) }
    var bot2 map[string]any
    json.NewDecoder(resU.Body).Decode(&bot2)
    resU.Body.Close()
    if v, ok := bot2["secure_embed_enabled"].(bool); !ok || !v { t.Fatalf("secure_embed_enabled not true") }
    ...
    if resG.StatusCode != http.StatusOK { t.Fatalf("expected 200, got %d", resG.StatusCode) }
    var bot3 map[string]any
    json.NewDecoder(resG.Body).Decode(&bot3)
    resG.Body.Close()
    if v, ok := bot3["secure_embed_enabled"].(bool); !ok || !v { t.Fatalf("secure_embed_enabled not persisted") }
}
