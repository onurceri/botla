...
func TestRateLimit_PerUserIsolation(t *testing.T) {
    ...
    if resA1.StatusCode != http.StatusOK { t.Fatalf("expected 200, got %d", resA1.StatusCode) }
    resA1.Body.Close()
    ...
    if resB1.StatusCode != http.StatusOK { t.Fatalf("expected 200, got %d", resB1.StatusCode) }
    resB1.Body.Close()
    ...
    if resB2.StatusCode != http.StatusTooManyRequests { t.Fatalf("expected 429, got %d", resB2.StatusCode) }
    resB2.Body.Close()
}
