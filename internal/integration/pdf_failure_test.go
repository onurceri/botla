...
func TestSources_PDF_Ingest_Failure_NoFitz(t *testing.T) {
    ...
    for i := 0; i < 50; i++ {
        ...
        if resG.StatusCode != http.StatusOK { resG.Body.Close(); continue }
        var st map[string]any
        json.NewDecoder(resG.Body).Decode(&st)
        resG.Body.Close()
        if st["status"].(string) == "failed" { failed = true; break }
    }
    if !failed { t.Fatalf("expected failed status without fitz") }
}
