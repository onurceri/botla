...
func TestAuth_EdgeCases(t *testing.T) {
    ...
    if res.StatusCode != http.StatusMethodNotAllowed { t.Fatalf("expected 405, got %d", res.StatusCode) }
    res.Body.Close()
    ...
    if r1.StatusCode != http.StatusBadRequest { t.Fatalf("expected 400, got %d", r1.StatusCode) }
    r1.Body.Close()
    ...
    if r2.StatusCode != http.StatusBadRequest { t.Fatalf("expected 400, got %d", r2.StatusCode) }
    r2.Body.Close()
