...
func TestPublicChatbotConfig_IncludesSuggestions(t *testing.T) {
    ...
    botID, err := db.CreateChatbot(nil, te.DB, bot)
    if err != nil { t.Fatalf("create bot: %v", err) }
...
func createTestUser(t *testing.T, db *sql.DB) string {
    var id string
    if err := db.QueryRow(`INSERT INTO users (email, password_hash) VALUES ($1,$2) RETURNING id`, "test@example.com", "x").Scan(&id); err != nil {
