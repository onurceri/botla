...
func TestAnalytics_ThumbsUpAfterFeedback(t *testing.T) {
    ...
    if resF.StatusCode != http.StatusOK { t.Fatalf("expected 200, got %d", resF.StatusCode) }
    resF.Body.Close()
    ...
    if resA.StatusCode != http.StatusOK { t.Fatalf("expected 200, got %d", resA.StatusCode) }
    ...
    if sumThumbs == 0 {
        ...
        if err != nil { t.Fatalf("analytics query err: %v", err) }
        sumThumbs = int(cnt.Int64)
    }
    if sumThumbs < 1 { t.Fatalf("expected thumbs_up_count >=1, got %d", sumThumbs) }
}
