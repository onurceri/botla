...
func TestAnalytics_UpdatesAfterChat(t *testing.T) {
    ...
    var series []struct{ Date string; Messages int; Conversations int }
    json.NewDecoder(resA.Body).Decode(&series)
    resA.Body.Close()
    var totalM, totalC int
    for _, p := range series { totalM += p.Messages; totalC += p.Conversations }
    if totalM < 2 || totalC < 1 { t.Fatalf("expected totals >= (2 messages, 1 conv), got (%d, %d)", totalM, totalC) }
}
