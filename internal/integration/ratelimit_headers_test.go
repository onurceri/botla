...
func TestRateLimit_Headers(t *testing.T) {
    ...
    if res1.StatusCode != http.StatusOK { t.Fatalf("expected 200, got %d", res1.StatusCode) }
    if lim := res1.Header.Get("X-RateLimit-Limit"); lim != "2" { t.Fatalf("limit header expected 2, got %q", lim) }
    if rem := res1.Header.Get("X-RateLimit-Remaining"); rem != "1" { t.Fatalf("remaining expected 1, got %q", rem) }
    res1.Body.Close()
    ...
    if res2.StatusCode != http.StatusOK { t.Fatalf("expected 200, got %d", res2.StatusCode) }
    if rem := res2.Header.Get("X-RateLimit-Remaining"); rem != "0" { t.Fatalf("remaining expected 0, got %q", rem) }
    res2.Body.Close()
    ...
    if res3.StatusCode != http.StatusTooManyRequests { t.Fatalf("expected 429, got %d", res3.StatusCode) }
    ra := res3.Header.Get("Retry-After")
    if ra == "" { t.Fatalf("missing Retry-After header") }
    if n, err := strconv.Atoi(ra); err != nil || n <= 0 { t.Fatalf("invalid Retry-After: %q", ra) }
    res3.Body.Close()
}
