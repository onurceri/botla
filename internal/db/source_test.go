...
func TestDataSource_CRUD_DB(t *testing.T) {
	...
	sid, err := db.CreateDataSource(context.Background(), dbConn, s)
	if err != nil || sid == "" {
		t.Fatalf("create source: %v", err)
	}
	// list
	list, err := db.ListSourcesByChatbotID(context.Background(), dbConn, bid)
	...
	now := time.Now()
	em := ""
	if err2 := db.UpdateSourceProcessing(context.Background(), dbConn, sid, "completed", &em, 3, &now); err2 != nil {
		t.Fatalf("update processing: %v", err2)
	}
	got, err2 := db.GetSourceByID(context.Background(), dbConn, sid)
	if err2 != nil || got == nil || got.Status != "completed" {
		t.Fatalf("get source: %v", err2)
	}
	// update capability and suggestions
	if err3 := db.UpdateSourceCapability(context.Background(), dbConn, sid, "summary"); err3 != nil {
		t.Fatalf("cap: %v", err3)
	}
	if err3 := db.UpdateSourceSuggestions(context.Background(), dbConn, sid, []string{"q1", "q2"}); err3 != nil {
		t.Fatalf("sugg: %v", err3)
	}
	// delete
	if err3 := db.DeleteSource(context.Background(), dbConn, sid); err3 != nil {
		t.Fatalf("delete: %v", err3)
	}
	gone, err3 := db.GetSourceByID(context.Background(), dbConn, sid)
	if err3 != nil {
