...
func TestWorkerPool_MultipleWorkers(t *testing.T) {
	...
	q, err := StartSourceQueue(nil, nil, nil, nil, nil, storage.NewMemoryStorage(), nil, mockVC, nil, nil, 5)
	if err != nil {
...
func TestWorkerPool_MaxWorkerLimit(t *testing.T) {
	...
	q, err := StartSourceQueue(nil, nil, nil, nil, nil, storage.NewMemoryStorage(), nil, mockVC, nil, nil, 20)
	if err != nil {
		t.Fatalf("StartSourceQueue failed: %v", err)
	}
	defer q.Stop()
