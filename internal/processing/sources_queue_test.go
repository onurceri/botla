...
func TestAggregateAndPersistChatbotSuggestions(t *testing.T) {
    ...
    if err := db.QueryRow(`INSERT INTO users (email, password_hash) VALUES ($1,$2) RETURNING id`, email, "x").Scan(&uid); err != nil { t.Fatalf("user: %v", err) }
    if err := db.QueryRow(`INSERT INTO chatbots (user_id, name) VALUES ($1,$2) RETURNING id`, uid, "X").Scan(&bid); err != nil { t.Fatalf("bot: %v", err) }
    // insert sources
    arr1, _ := json.Marshal([]string{"S1", "S2"})
    arr2, _ := json.Marshal([]string{"S2", "S3"})
    if _, err := db.Exec(`INSERT INTO data_sources (chatbot_id, source_type, suggested_questions) VALUES ($1,'text',$2)`, bid, arr1); err != nil { t.Fatalf("src1: %v", err) }
    if _, err := db.Exec(`INSERT INTO data_sources (chatbot_id, source_type, suggested_questions) VALUES ($1,'url',$2)`, bid, arr2); err != nil { t.Fatalf("src2: %v", err) }
    aggregateAndPersistChatbotSuggestions(context.Background(), db, bid)
    var js []byte
    if err := db.QueryRow(`SELECT suggested_questions FROM chatbots WHERE id=$1`, bid).Scan(&js); err != nil { t.Fatalf("chatbot: %v", err) }
    var out []string
    _ = json.Unmarshal(js, &out)
    // We expect S1, S2 (from existing) + S3 (from new source) = 3 total
    if len(out) != 3 { t.Fatalf("expected 3 merged suggestions, got %d: %v", len(out), out) }
    // Check content
    hasS3 := false
    for _, s := range out { if s == "S3" { hasS3 = true } }
    if !hasS3 { t.Fatalf("expected S3 in suggestions, got %v", out) }
}
